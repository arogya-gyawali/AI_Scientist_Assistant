...
8 stages, all reading from and writing to one shared `ExperimentPlan` document. Today the document lives as a JSON file under `plans/` (see [`src/lib/plan.py`](../src/lib/plan.py)); Supabase JSONB is the planned upgrade. No stage-to-stage handoffs — every stage's data dependency is on fields of the shared plan. The orchestrator schedules a stage when its `reads` are populated and the stage isn't already running or complete.
...
| Stage | Reads | Writes | Parallel-safe | Status |
|---|---|---|---|---|
| 1. Lit Review | `hypothesis` | `lit_review` | yes | ✅ shipped |
| 2. Protocol | `hypothesis` | `protocol` | yes | ✅ shipped |
| 3. Materials | `protocol` | `materials` | yes | ✅ shipped |
| 5. Timeline | `protocol` | `timeline` | yes | ✅ shipped |
| 6. Validation | `hypothesis`, `protocol` | `validation` | yes | ✅ shipped |
| 4. Budget | `materials` | `budget` | yes | ⏳ pending |
| 7. Design Critique | `hypothesis`, `lit_review`, `protocol`, `materials`, `budget`, `timeline`, `validation` | `critique` | yes | ✅ shipped (reads `budget` as optional) |
| 8. Summary | all 8 stage fields | `summary` | no (last) | ⏳ pending |
